 ...
 * 内置方法
 ******************************************************************************/
make([]int, 2,4)	//分配内存，make 用于内置引用类型（切片、map 和管道），返回的是类型本身
make(chan,2)
make(map[string]string, 0)

len("abc")	//len 用于返回某个类型的长度或数量（字符串、数组、切片、map 和管道）
cap([]int{})	//cap 是容量的意思，用于返回某个类型的最大容量（只能用于切片和 map）
new(int)	//分配内存，new 用于值类型和用户定义的类型，如自定义结构，返回内存地址指针，分配的内存置为0
append([]int{},[]int{})	//连接切片
copy(dst []int,src []int)	//用于复制切片
close(ch chan)	//用于关闭管道通信，用在发送侧
delete(map[type]type,key)	//删除map中元素
complex(r int,i int)	//创建复数
real(c complex128)	//返回复数实数
imag(c complex128)	//返回复数虚数
panic(T)	//异常处理
recover(T)	//捕抓异常，需要与panic同线程
